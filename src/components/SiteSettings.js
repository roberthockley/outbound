...
export const SiteSettings = () => {
    ...
    const getHours = (site) => {
        campaigns2 = JSON.parse(localStorage.getItem('campaigns'))
        setDynamoCampaigns(JSON.parse(localStorage.getItem('campaigns')))

        for (let i = 0; i < campaigns2.length; i++) {
            if (campaigns2[i].campaign === site) {
                if (campaigns2[i].shift === "siesta") {
                    setMultiTimes(true)
                } else {
                    setMultiTimes(false)
                }
                if (campaigns2[i].hours.length === 0) {
                    setShowTimes(true)
                }
                else {
                    setShowTimes(true)
                    ...
                    let timesToUse = campaigns2[i].hours
        ...
        if (save === true) {
            ...
            for (let i = 0; i < dynamoCampaigns.length; i++) {
                if (dynamoCampaigns[i].campaign === siteCampaign) {
                    dynamoCampaigns[i].hours = result
                }
            }
            console.log(dynamoCampaigns)
